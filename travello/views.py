...
from django.core.mail import send_mail      #mail package for sending original mail
...
def upcoming_trips(request):
    ...
    date1=datetime.now().date()
    person = pessanger_detail.objects.all().filter(username=username).filter(pay_done=0)
...
def card_payment(request):
    card_no = request.POST['card_number']
    pay_method = 'Debit card'
    MM = request.POST['MM']
    YY = request.POST['YY']
    CVV = request.POST['cvv']
    request.session['dcard'] = card_no
    balance=100000*int(MM)*int(YY)
    request.session['total_balance'] = balance
    username = request.user.get_username()
    user = User.objects.get(username=username)
    mail_id = user.email
    pay_method = 'Card'
    balance_data = Cards(Card_number=card_no,Ex_month=MM,Ex_Year=YY,CVV=CVV,Balance=balance,email=mail_id)
    balance_data.save()
    if  pay_method == 'Card':
        rno = random.randint(100000, 999999)  #generate Random Otp
        ...
        msg = 'Your OTP For Payment of ₹' + str(amt) + ' is ' + str(rno)
                ...
                # print([mail_id])
                # print(amt)
        send_mail('OTP for Debit card Payment',msg,'karishmapatel.hnb@gmail.com',[mail_id],fail_silently=False)
        #send_mail('title',msg,'admin email',[user email id],fail_silently=error showing data)
...
def net_payment(request):
    ...
    pay_method = 'NetBanking'
    if pay_method == 'NetBanking':
        netdata=NetBanking(Username=usernameget,Password=Password1,Bank=Bank_name,Balance=amt)
        netdata.save()
